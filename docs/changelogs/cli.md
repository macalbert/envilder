...
* **Reject map-file mapping keys that could inject extra `.env` lines**: The
  CLI now rejects empty or whitespace-only variable names and names containing
  `=`, carriage return, newline, or the `U+2028`/`U+2029` line separators when
  parsing a map file or writing an environment file, instead of silently
  writing them through. A name embedding one of these characters could smuggle
  an extra `key=value` assignment into the generated environment file
