...
export async function fetchSSMParameter(ssmPath: string): Promise<string | undefined> {
  const ssm = new SSM();
  const params = {
    Name: ssmPath,
    WithDecryption: true,
...
export async function generateEnvFile(paramMap: Record<string, string>, envFilePath: string) {
...
if (require.main === module) {
  ...
  if (!paramMapPath || !envFilePath) {
    console.error('Usage: npm run envilder --map=param_map.json --envfile=.env');
    process.exit(1);
  }

  const paramMap = JSON.parse(fs.readFileSync(paramMapPath, 'utf-8'));

  generateEnvFile(paramMap, envFilePath).catch((err) => console.error('Error generating .env file:', err));
}
